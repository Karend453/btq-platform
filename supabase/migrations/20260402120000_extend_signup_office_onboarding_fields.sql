-- Onboarding fields for Lofty / Landvoice setup (signup RPC).

ALTER TABLE public.offices
  ADD COLUMN IF NOT EXISTS mls_url text,
  ADD COLUMN IF NOT EXISTS landvoice_leads text,
  ADD COLUMN IF NOT EXISTS referral_source text;

DROP FUNCTION IF EXISTS public.complete_broker_signup (text, text, text, text, text);
