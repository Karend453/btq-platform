-- Office-owned checklist templates, BTQ starters (office_id IS NULL), clone on office creation, RLS.

-- ---------------------------------------------------------------------------
-- checklist_templates: ownership, archive, type, defaults, traceability
-- ---------------------------------------------------------------------------
...
UPDATE public.checklist_templates
SET created_from = 'btq_starter'
WHERE office_id IS NULL AND created_from = 'manual';
...
DROP INDEX IF EXISTS idx_checklist_templates_one_default_per_office_type;
...
-- ---------------------------------------------------------------------------
-- Seed minimal BTQ starters when none exist (office_id IS NULL)
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  n int;
...
BEGIN
  ...
  WHERE office_id IS NULL AND archived_at IS NULL;
  ...
  INSERT INTO public.checklist_templates (
    id, name, office_id, archived_at, checklist_type, is_default_for_type,
    source_template_id, created_at, created_from
  ) VALUES
    (tid_purchase, 'BTQ Purchase', NULL, NULL, 'Purchase', true, NULL, now(), 'btq_starter'),
    (tid_listing, 'BTQ Listing', NULL, NULL, 'Listing', true, NULL, now(), 'btq_starter'),
    (tid_lease, 'BTQ Lease', NULL, NULL, 'Lease', true, NULL, now(), 'btq_starter'),
    (tid_other, 'BTQ Other', NULL, NULL, 'Other', true, NULL, now(), 'btq_starter');
...
-- ---------------------------------------------------------------------------
-- Clone BTQ starters into a new office (SECURITY DEFINER)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF src_count = 0 THEN
    RAISE EXCEPTION 'No BTQ starter templates available to clone (expected rows with office_id IS NULL).';
  END IF;
...
-- ---------------------------------------------------------------------------
-- Atomic default swap (unset others for same office + type, then set one)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Trigger: new office → clone BTQ templates (insert fails if clone raises)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- RLS: office-scoped reads; broker/admin mutate office-owned rows only
-- ---------------------------------------------------------------------------
...
-- Sections / items: same office gate via parent template
